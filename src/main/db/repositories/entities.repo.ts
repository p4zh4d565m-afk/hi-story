...
export class CharacterRepo {
  ...
  create(input: CreateCharacterInput): IpcResult<Character> {
    ...
    this.db.prepare(`
      INSERT INTO characters (id, project_id, name, aliases, appearance, personality, background, arc, sort_order, created_at, updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)
  ...
  update(input: UpdateCharacterInput): IpcResult<Character> {
    ...
    this.db.prepare(`UPDATE characters SET name=?, aliases=?, appearance=?, personality=?, background=?, arc=?, updated_at=? WHERE id=?`)
      .run(input.name ?? ch.name, input.aliases ?? ch.aliases, input.appearance ?? ch.appearance,
           input.personality ?? ch.personality, input.background ?? ch.background, input.arc ?? ch.arc, now, input.id);
...
function rowToCharacter(row: Record<string, unknown>): Character {
  return { id: row.id as string, projectId: row.project_id as string, name: row.name as string,
    ...
    background: row.background as string, arc: row.arc as string, sortOrder: row.sort_order as number,
...
export class ReferenceLinkRepo {
  ...
  create(input: CreateReferenceLinkInput): IpcResult<any> {
    ...
    this.db.prepare(`INSERT INTO reference_links (id, source_type, source_id, target_type, target_id, relation_type, created_at)
      VALUES (?,?,?,?,?,?,?)`).run(id, input.sourceType, input.sourceId, input.targetType, input.targetId, input.relationType, now);
    ...
    return { success: true, data: { id: row.id, sourceType: row.source_type, sourceId: row.source_id,
      targetType: row.target_type, targetId: row.target_id, relationType: row.relation_type, createdAt: row.created_at } };
  }
  ...
  findBySource(sourceType: string, sourceId: string): IpcResult<any[]> {
    ...
    return { success: true, data: rows.map(r => ({ id: r.id, sourceType: r.source_type, sourceId: r.source_id, targetType: r.target_type, targetId: r.target_id, relationType: r.relation_type, createdAt: r.created_at })) };
  }
