...
- 单个词/句式**单次出现**且上下文自然：放行，不机械替换
- **同一段内聚集**（如「然而」反复、「不仅……而且……」连用、「如同……一般」扎堆）：这才是 AI 味信号，必须规避
- 规避时优先换掉**最不贴切**的一两处，保留最自然的，不是全部回避

### 误杀防护（去 AI 味 ≠ 磨平一切，吸收自 shuorenhua 的 protected spans）

以下内容即使"眼熟"也要照常写，不要因为去 AI 味而改掉：

- **角色台词的口吻**：每个角色的语气、口头禅、说话节奏是人格的一部分。去 AI 味针对叙述文字，不针对角色台词，不要把所有角色都写成规整书面腔
- **专有名词与设定**：人名、地名、势力名、功法名等设定词，以及已建立的世界观规则，该用就用、该重复就重复，不要为了"去重"而换说法
- **伏笔与关键句**：该重复的伏笔、口号、标志性台词，重复本身是叙事需要，不是 AI 味

### 注入灵魂（去 AI 味 ≠ 无菌干净，吸收自 humanizer-zh）

删掉模板句式只是及格线。如果写出来长短划一、没有态度、像中立新闻稿，依然一眼是 AI。好文字背后有真实的人：

- **叙述有反应**：不当摄像头。叙述者对情节做出情绪反应，把反应融进动作和细节
...
## 写完两遍自查（输出前必须做，吸收自 shuorenhua）

**第一遍 · 保真回查**：对照大纲、角色设定、世界观和前文事实——
- 情节是否符合本章大纲？有没有漏掉关键节点？
- 角色行为、对话是否符合人设？有没有 OOC 或信息越界？
- 世界观规则、角色能力有没有前后矛盾？
- 前文已建立的设定、事实有没有被无意改动？
任何一条不达标，先修正一致性问题。

**第二遍 · 残留味回查**：确认保真后，再查这一章还有没有 AI 味——
- 开头是否用天气/起床/回顾上一章这类慢热开场？
- 是否有「值得注意的是」「在这个时间点」「综上所述」这类提示层或空收尾？
- 是否每段都差不多长、每句都差不多抬手落点（被统一抛光）？
- 章末是否又强行升华成金句？
发现残留，局部改写即可，不要推倒重写。`;
