...
- 创作决策确认账本已完成：迁移 v18、四类严格载荷、assistant 来源归属校验、单事务批量确认、effect 审计、幂等、拒绝和四类修订均已落地；迁移 DDL 与版本登记原子提交，同一父决策禁止并行待确认修订。AI 提取仅保存 `proposed`，确认失败保留候选重试。
...
- 账本专项：7 个测试文件、31 项测试通过。
- 真实 UI：写作工作区 10/10，创作决策 8/8 通过。
- `npm run test`：21 个测试文件、92 项测试通过；`npm run build` 通过。
