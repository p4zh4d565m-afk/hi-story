...
- 任务 #144 / #145 / #146 与 AI 对话清理（v22）已收口；第一版稳定写作基线见标签 `writing-baseline-v1`。
- **#147 与其余 P2** 已转入**边写边维护**（不再阻塞写作）：先拆清单，优先数据安全、上下文正确性、后续开发阻断项。
- 顺带发现的产品体验问题（非本轮范围）：对话按章隔离等仍可另立项；v22 已支持按轮删/清空与短时撤销。
- 长期：清理 D 盘根目录混入的 Node 安装，或把正确的 Node 目录排到 PATH 前面；日常启动用 `Launch hi story.bat`。
