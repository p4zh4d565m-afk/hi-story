...
- **审稿版本账本（chapter-run 二期，2026-09-15）** — 迁移 v23：`chapters.content_generation`（世代）+ `chapter_reviews` + `chapter_revision_proposals`（同章一个 proposed 唯一索引）。核心：`content-revision.ts` 的 `normalizeChapterText`/`shouldBumpContentGeneration`（等价 HTML 不递增、改字/标点递增）；`chapter.repo.ts` 世代递增与打历史快照判据**分开**（世代用 normalize、快照暂留 `content !==`）；审稿 prompt 三态 `pass/issue/inconclusive` + `evidence` + `issues[]`（`REVIEW_PROMPT_VERSION = review-v2-2026-09-15`）；主进程 `chapter-review.service.ts` 解析（15 维度完整性校验）/聚合（coverage ≥12/15、critical 硬门禁、70 分门槛）/`applyRevision`（世代冲突标 stale 在**事务外**，避免被回滚）；workflow IPC `run`（一次性 chat + 双重世代校验 + 同章互斥 + 可取消）/`list`（含 freshness、pendingReReview）/`createRevision`（取消不落 proposed）/`applyRevision`/`rejectRevision`。渲染端 AIReviewPanel 改走 workflow（不再自开 chatStream/自解析/直写 update），加审稿历史列表 + 待复评提示。报告 `docs/reports/2026-09-15-chapter-run-phase2-report.md`。
- **写章运行记录（chapter-run 三期，2026-09-15）** — 迁移 v24：`chapter_runs` 一张表（状态机 `running/drafted/committed/failed/cancelled` + `draft_content` + `input_summary` + `extract_status`）。核心：`chapter-run.service.ts` 的 `commit`（幂等，重复返回原章节）+ `recoverInterruptedAll`（启动扫 `running` → `failed/PROCESS_INTERRUPTED`，不自动重放付费请求）；workflow IPC `start`（一次性 chat + 落草稿）/`cancel`（绑定 projectId + runId，`cancelByRunId`）/`commit`/`get`/`list`。渲染端 AIWritePanel 单章改走 `start → drafted → commit` 两段式（放弃逐字流式），保存走幂等 commit；`App.tsx` 加 `onChapterCommitted` 回写完整章节。**批量写章暂不接 run（继续 localStorage 断点续写，另立 P2）；抽取重试 `retryExtract` IPC 未实现（字段已建）**。报告 `docs/reports/2026-09-15-chapter-run-phase3-report.md`。
