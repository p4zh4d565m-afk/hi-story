...
- **AI 流真实取消（一期，2026-09-12）** — 对应 Spec `docs/superpowers/specs/2026-09-10-chapter-run-review-ledger-design.md` 一期。`ChatOptions` 加 `signal`：Claude `messages.stream(body, {signal})` + `stream.abort()`，OpenAI 兼容路径把用户 signal 与超时 abort 合并（任一方 abort 都取消），取消归一为 `AI_STREAM_CANCELLED`（不显示成普通网络失败）。主进程 `stream-registry.ts` 登记 `streamId + projectId + senderId + AbortController`，`ai:cancelStream` 先标 terminal 再 abort，Token/complete/error 回调前查 terminal 与 sender 存活；`sender.once('destroyed')` 中止该窗口全部流。渲染端 `ai.service` 维护 `streamId → {projectId, ignored, cancelled, wake}` 映射，`cancelActiveStreams(projectId)` 精确取消并唤醒对应 generator 抛「已停止，不会保存」（主进程取消后不再发事件，必须主动唤醒，否则 for-await 永远挂住）；`ignoreProjectStreams(projectId)` 只作废不 abort（切项目不误杀后台），App 层切项目时作废旧项目流。四面板接取消：AIWritePanel 停止按钮真实取消 + 批量流带 projectId、AIChatPanel 加停止按钮 + 两处流带 projectId、AIReviewPanel/AIPolishPanel 流带 projectId + 停止态静默。修复一处真实 bug：complete 与最后 token 同 tick 到达时旧循环 `while(!finished)` 会丢 pendingTokens，改为先 drain 再判 finished。单测 332（新增 stream-registry 5 + ai-service 4）、三套 UI 回归 12/16/114 全过。
