...
- **工作区 P3 拖放进槽（2026-09-12 已编码，2026-09-13 翻案后未标落地）** — `WorkspaceLayoutV1` 管大纲/素材/伏笔/灵感/参考/起名入 right + 侧栏归属；**写章/审稿/润色改回浮动窗**（产品实测推翻「3+3 放 bottom」，走布尔 + `display:none` 保活）；AI 对话不迁。拖拽 HTML5 drag（SlotTabs 标签 + 槽 onDrop）；空 bottom 有面板才挂 Separator + 生效 `BOTTOM_MIN_PX`；顶栏 `openOrFocus` 不再把已拖走面板拽回默认槽。**残留/技术债**：侧栏 `layout.slots.left` 未消费；单面板无拖拽入口、空 right 无 drop 目标（拖拽可用性第二批未做）；拖拽无 Electron E2E。核心在 `src/renderer/workspace/layout-model.ts` + `useWorkspaceLayout.ts`。单测 `tests/unit/workspace-layout.test.ts` 16/16，写作 UI 12/12。
