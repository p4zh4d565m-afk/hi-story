...
- **工作区 P1 主题（2026-09-12）** — CSS 变量令牌 + 顶栏浅色/深色切换（`hi-story-theme`）；Dark 外观与旧色板一致。写章/审稿/润色灰底与按钮显式白字未扫。**下一步 P2**：`react-resizable-panels` 替换手写 splitter、加 bottom 槽与边轨折叠；计划 `docs/superpowers/plans/2026-09-12-workspace-p2-splitter.md`。P1 尚未 commit，P2 开工前须单独提交，禁止与 splitter 混改。
