...
- **工作区 P3 拖放进槽（2026-09-12，已编码未标落地）** — 六个浮窗入槽 + `WorkspaceLayoutV1`（`hi-story-workspace-v1` 管归属，`useDefaultLayout` 管比例）。大纲/素材/伏笔默认 right、写章/审稿/润色默认 bottom（保活面板固定 bottom 不走 `movePanel`，组件始终挂载 display 切显隐）；AI 对话本轮不迁。App 废弃 9 个 `panelState` 布尔。槽内普通面板全挂载切标签保 state；拖拽用 HTML5 drag（SlotTabs 标签 + 槽容器 onDrop），保活面板标签不 draggable。核心在 `src/renderer/workspace/layout-model.ts` + `useWorkspaceLayout.ts`。单测 `tests/unit/workspace-layout.test.ts` 22/22，写作 UI 12/12。**残留/技术债**：拖拽与生成中保活未真窗口手测；侧栏 `layout.slots.left` 未消费；三 AI 面板 `if(!open) return null` 残留。详见工作区 `workspace-p3-dock-report.md`。
