// 独立隐藏窗口运行真实编辑器回归，不加载应用主进程或用户数据库。
const path = require('node:path');
const fs = require('node:fs');
const os = require('node:os');

if (!process.versions.electron) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hi-story-writing-test-'));
  require('esbuild').buildSync({
    entryPoints: [path.join(__dirname, 'writing-workspace.tsx')],
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      const results = await win.webContents.executeJavaScript('window.runWritingRegression()');
      for (const result of results) console.log(`${result.error ? '失败' : '通过'}：${result.name}${result.error ? ' — ' + result.error : ''}`);
      const failures = results.filter(result => result.error).length;
      console.log(`界面回归：${results.length - failures}/${results.length} 通过`);
